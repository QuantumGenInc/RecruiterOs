import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/prisma";
import EmailFeed from "@/components/EmailFeed";

export default async function EmailFeedPage() {
  const session = await getServerSession(authOptions);
  if (!session) redirect("/login");
  if (session.user.role !== "RECRUITER") redirect("/dashboard/candidate");

  const profile = await prisma.recruiterProfile.findUnique({
    where: { userId: session.user.id },
    include: {
      rawEmails: { orderBy: { receivedAt: "desc" }, take: 50 },
    },
  });

  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-2">Email Feed</h1>
      <p className="text-sm text-gray-400 mb-6">
        All recruiter emails and their AI classifications
      </p>
      {profile?.rawEmails && profile.rawEmails.length > 0 ? (
        <EmailFeed emails={profile.rawEmails} />
      ) : (
        <p className="text-sm text-gray-500 py-12 text-center">
          No emails yet. Connect Gmail and poll to start.
        </p>
      )}
    </div>
  );
}
